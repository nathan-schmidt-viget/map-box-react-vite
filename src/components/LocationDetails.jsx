...
const LocationDetails = ({ geoMapItem, isLoading, locationPopUp, setLocationPopUp }) => {
  ...
  const locationDetailsPopUpWrapper = useRef(null);

  // when open changes run open/close command
  useEffect(() => {
    const { current: el } = locationDetailsPopUp;
    locationPopUp ? el.showModal() : el.close();
  }, [locationPopUp]);

  const closeDialog = (e) => {
    if (e.target == locationDetailsPopUp.current) {
  ...
  return (
    ...
    >
      <div ref={locationDetailsPopUpWrapper}>
        {isLoading && <p className="loading">Loading...</p>}

        <form method="dialog" className="flex justify-end absolute top-1 right-1 z-10 ">
          <button className="w-8 h-8 text-white bg-zinc-900 rounded hover:text-zinc-900 hover:bg-white focus:text-zinc-900 focus:bg-white" onClick={() => setLocationPopUp(!locationPopUp)}>
            <CloseSvg />
